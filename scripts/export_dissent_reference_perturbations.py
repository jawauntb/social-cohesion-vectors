"""Export perturbations for an external generated dissent residual."""
...
from social_cohesion_vectors.experiments.dissent_reference_perturbation import (
    DISSENT_REFERENCE_BASE_CONTRAST_ID,
    export_dissent_reference_perturbations,
)


def main(argv: Sequence[str] | None = None) -> int:
    args = _parse_args(argv)
    counts, report = export_dissent_reference_perturbations(
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=DISSENT_REFERENCE_BASE_CONTRAST_ID,
    )
    parser.add_argument(
        "--pairs-output",
        type=Path,
        default=paths.training / "dissent_reference_perturbation_pairs.jsonl",
    )
    parser.add_argument(
        "--prompts-output",
        type=Path,
        default=paths.training / "dissent_reference_perturbation_prompts.jsonl",
    )
    parser.add_argument(
        "--json-report-output",
        type=Path,
        default=paths.reports / "dissent_reference_perturbation.json",
    )
    parser.add_argument(
        "--markdown-report-output",
        type=Path,
        default=paths.reports / "dissent_reference_perturbation.md",
    )
