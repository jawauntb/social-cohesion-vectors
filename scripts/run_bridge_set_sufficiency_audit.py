"""Run intentional bridge-set sufficiency audits."""
...
from social_cohesion_vectors.experiments.heldout_domain_direction_audit import (
    ...
    save_bridge_set_sufficiency_audit_report,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        ...
        f"ready={summary['ready_for_bridge_set_claims']} "
        f"source_min_margin={summary['source_min_margin']:+.3f} "
        f"target_min_margin={summary['target_min_margin']:+.3f} "
        f"failed_folds={summary['failed_fold_count']}"
    )
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--json-output",
        type=Path,
        default=paths.reports / "bridge_set_sufficiency_audit.json",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=paths.reports / "bridge_set_sufficiency_audit.md",
    )
