"""Trace hidden projection movement during Modal activation steering."""
...
from social_cohesion_vectors.experiments.steering_telemetry import (
    write_steering_telemetry_reports,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        "tracing steered generation on Modal "
        f"model={args.model_id} layer={args.layer} prompts={len(prompts)} "
        f"strengths={args.strengths}"
    )
    with app.run():
        traces = trace_activation_steering.remote(
            records=prompts,
    ...
    write_jsonl(traces, args.traces_output)
    report = write_steering_telemetry_reports(
        traces,
    ...
    if not isinstance(summary, dict):
        raise TypeError("telemetry report summary is not a dictionary")
    print(
        "steering telemetry: "
        f"delta_error={float(summary['mean_absolute_delta_error']):.6f} "
        f"post_projection_pos_neg={float(summary['positive_minus_negative_post_projection_delta']):+.4f} "
        f"score_pos_neg={float(summary['positive_minus_negative_score_delta']):+.4f}"
    )
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--traces-output",
        type=Path,
        default=config.paths.processed / "steering_hidden_projection_traces.jsonl",
    )
    parser.add_argument(
        "--json-output",
        type=Path,
        default=config.paths.reports / "steering_hidden_projection_telemetry.json",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=config.paths.reports / "steering_hidden_projection_telemetry.md",
    )
